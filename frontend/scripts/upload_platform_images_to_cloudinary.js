  ...
  return new Promise((resolve, reject) => {
    cloudinary.uploader.upload(
      localFilePath,
      {
        ...
        // Ensure highest quality preservation
...
async function run() {
  ...
  const platformMap = {};
  const mapFilePath = path.join(__dirname, '..', 'src', 'lib', 'cloudinary_platform_images.json');
  if (fs.existsSync(mapFilePath)) {
    try {
      Object.assign(platformMap, JSON.parse(fs.readFileSync(mapFilePath, 'utf8')));
    } catch (e) {}
  }

  let uploadSuccess = 0;
  for (const filePath of allImages) {
    const relFromPublic = path.relative(path.join(__dirname, '..', 'public'), filePath).replace(/\\/g, '/');
    const localUrl = '/' + relFromPublic;
    
    // Determine publicId and folder
